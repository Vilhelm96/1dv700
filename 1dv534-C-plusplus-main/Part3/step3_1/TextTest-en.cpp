...
// Authors:	Christer Lundberg (& Vilhelm Park will use this for this assingment)
// ------------------------------------------
...
int App::run()
{
	...
	try {
		//Changed from Texthandler to TextHandler since I misspelled my clas.
		...
		for (int i = 0; i < currentNrOfTexts; ++i) {
			...
			if (nrOfChars != check[i+1]) {
				cout << "ERROR: Length of text should have been " << check[i + 1]
				<< "!" << endl;
			}
			cout << "----------------------------------------------------\n";
